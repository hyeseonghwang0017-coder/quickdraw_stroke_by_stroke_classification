...
1. **최소 완성도 비율** — 현재 획 수 / 전체 획 수가 이 비율 미만이면 예측을 건너뜁니다.
   - 1차: train 데이터에서 클래스별 평균 획수를 구해 `0.30 + (정규화값 × 0.30)` 으로 초기값 설정 (범위 0.30~0.60)
   - 2차: Val 데이터에서 모델이 신뢰도 ≥ 0.90으로 처음 정답을 맞춘 시점의 완성도를 수집하여 40th percentile로 최종값 결정

2. **최소 획수** — 예측 후보 클래스별로 지금까지 그려진 획 수가 이 값 미만이면 해당 클래스를 후보에서 제외합니다.
   - train 데이터 획 수 분포의 10th percentile − 1로 클래스별 설정

3. **클래스별 최적 신뢰도 임계값** — Top-1 예측의 softmax 확률이 이 값 이상일 때만 예측을 확정합니다.
   - Val 데이터(10,000개 subset)에서 `[0.60, 0.65, ..., 0.95]` 8개 후보를 모두 시뮬레이션하여 클래스별로 정확도가 가장 높은 값을 선택
